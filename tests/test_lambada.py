...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "default",
            "I look at him, feeling stunned. Like this is some sort of",
            " sign",
        ),
        (
            "lambada_standard",
...
def test_lambada_scores_single_continuation_loglikelihood(
    monkeypatch,
    factory_name: str,
    doc: dict[str, str],
...
def test_lambada_marks_non_greedy_continuation_incorrect(monkeypatch) -> None:
    ...
    result = evalution.benchmarks.lambada_openai(max_rows=1, batch_size=6).evaluate(
        FakeSession(
            expected_prompt="The detective paused and finally saw the hidden",
