...
def test_openbookqa_scores_four_way_multiple_choice_accuracy(monkeypatch) -> None:
    """Verify openbookqa scores four way multiple choice accuracy. Keep the scoring path explicit so benchmark-specific behavior stays auditable."""
...
def test_openbookqa_can_emit_label_permutation_metric(monkeypatch) -> None:
    """Verify openbookqa can emit label permutation metric. Keep the scoring path explicit so benchmark-specific behavior stays auditable."""
    dataset = Dataset.from_list(
        [
            {
                "id": "obqa-1",
