...
def test_drop_scores_best_alias(monkeypatch) -> None:
    """Verify drop scores best alias. Keep the scoring path explicit so benchmark-specific behavior stays auditable."""
...
def test_drop_qa_text_scorer_uses_best_alias() -> None:
    """Verify drop QA text scorer uses best alias. Keep the scoring path explicit so benchmark-specific behavior stays auditable."""
