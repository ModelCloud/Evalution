...
- `engines.Tinygrad()`: the tinygrad-native local runtime backend
...
`engines.Tinygrad()` loads local checkpoints through tinygrad's own LLM runtime and keeps
generation, log-likelihood, and rolling log-likelihood fully in process. It accepts both direct
GGUF files and local dense Hugging Face `llama` checkpoints. For GGUF chat-template benchmarks
such as `gsm8k_platinum`, keep `model.path` pointed at the `.gguf` file and
`model.tokenizer_path` pointed at the matching dense tokenizer checkout. Dense native loading
currently targets local `llama` checkpoints with `config.json`; use GGUF for broader tinygrad
coverage. Evalution emulates `generate_continuous(...)` and `loglikelihood_continuous(...)` with
same-thread fixed batches. On CUDA it defaults to the profiled graph-free tinygrad runtime
(`jit=2`, `jitbeam=0`) for the local RTX 4090 and A100 families because `jit=1` hit CUDA graph
construction failures and `jitbeam=2` did not finish the startup sweep in time. Override `jit`
or `jitbeam` explicitly when you want to test a different tinygrad runtime profile.
